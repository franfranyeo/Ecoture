...
function MFAVerification({ userData, onCancel, onSuccess }) {
    ...
    const fetchMFAMethods = async () => {
        try {
            const methods = await authService.getMFAMethods(userData.userId);
            const activeMethods = Object.entries(methods)
                .filter(([key, value]) => value && key !== 'userId')
                .map(([key]) => key);

            setMFAMethods(activeMethods);
        } catch (error) {
            toast.error('Failed to fetch 2FA methods');
        }
    };
    ...
    const handleVerify = async () => {
        try {
            ...
            onSuccess(userData, response.accessToken);
        } catch (error) {
